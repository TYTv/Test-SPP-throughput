...
enum{
	P_UART_LSR_TX_FIFO_NOT_EMPTY                   = 0x0004, //'0':Data NOT available, '1':Data availabl
...
#define printf(...)                 wiced_printf(NULL, 0, __VA_ARGS__)
...
uint8_t SPP_NVRAM_ID = WICED_NVRAM_VSID_START;
#define SPP_NVRAM_ID_BLANK	1
#define SPP_NVRAM_ID_LIM	(SPP_NVRAM_ID_BLANK + 5)
...
#define app_delete_nvram(...)	wiced_hal_delete_nvram(__VA_ARGS__, NULL)
...
void application_init(void)
{
    ...
    for( uint8_t i = 0; i < SPP_MAX_CONNECTIONS; i++ ){
...
wiced_result_t app_management_callback(wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data)
{
    ...
    wiced_bt_device_link_keys_t			nv_read;

    WICED_BT_TRACE("app_management_callback %d\n", event);
    printf("app_management_callback %d\n", event);
    switch(event)
    {
...
//        app_write_nvram(SPP_NVRAM_ID, sizeof(wiced_bt_device_link_keys_t), &p_event_data->paired_device_link_keys_update);
    	...
    	for(uint8_t i=WICED_NVRAM_VSID_START; i<=SPP_NVRAM_ID_LIM; i++){
			if( app_read_nvram( i, &nv_read, sizeof(wiced_bt_device_link_keys_t) ) != 0 ){
				if( memcmp( &nv_read.bd_addr, &p_event_data->pairing_io_capabilities_br_edr_request.bd_addr, sizeof(wiced_bt_device_address_t) ) == 0 ){
			        app_write_nvram( i, sizeof(wiced_bt_device_link_keys_t), &p_event_data->paired_device_link_keys_update );
    	...
    	if( result != WICED_BT_SUCCESS ){
			for(uint8_t i=WICED_NVRAM_VSID_START; i<=SPP_NVRAM_ID_LIM; i++){
				if( app_read_nvram( i, &nv_read, sizeof(wiced_bt_device_link_keys_t) ) == 0 ){
					app_write_nvram( i, sizeof(wiced_bt_device_link_keys_t), &p_event_data->paired_device_link_keys_update );
					if( (i+1) > SPP_NVRAM_ID_LIM ){
						app_delete_nvram(WICED_NVRAM_VSID_START);
					}else{
						app_delete_nvram(i+1);
					}
					result = WICED_BT_SUCCESS;
					break;
				}
...
//    	memcpy( &tmp, &p_event_data->pairing_io_capabilities_br_edr_request.bd_addr, sizeof(tmp) );
//    	result = WICED_BT_ERROR;
//    	for(uint8_t i=WICED_NVRAM_VSID_START; i<=SPP_NVRAM_ID_LIM; i++){
//			if (app_read_nvram(i, &p_event_data->paired_device_link_keys_request, sizeof(wiced_bt_device_link_keys_t)) != 0){
//				if( memcmp( &tmp, &p_event_data->pairing_io_capabilities_br_edr_request.bd_addr, sizeof(tmp) ) == 0 ){
//					printf("BTM_PAIRING_IO_CAPABILITIES_REQUEST_EVT bda %B\n", p_event_data->pairing_io_capabilities_br_edr_request.bd_addr);
//					result = WICED_BT_SUCCESS;
//					break;
//				}
//			}
...
//    	result = WICED_BT_ERROR;
//    	for(uint8_t i=WICED_NVRAM_VSID_START; i<=SPP_NVRAM_ID_LIM; i++){
//    		if( memcmp( &SPP_NVRAM_BD_ADDR_LIST[i], &p_event_data->pairing_io_capabilities_br_edr_request.bd_addr, sizeof(wiced_bt_device_address_t) ) == 0 ){
//    			if (app_read_nvram(i, &p_event_data->paired_device_link_keys_request, sizeof(wiced_bt_device_link_keys_t)) != 0){
//    				result = WICED_BT_SUCCESS;
//    				break;
//    			}
...
//    	result = WICED_BT_ERROR;
//    	if( SPP_NVRAM_ID = id_search(&p_event_data->pairing_io_capabilities_br_edr_request.bd_addr != 0 ) ){
//			if( app_read_nvram(SPP_NVRAM_ID, &p_event_data->paired_device_link_keys_request, sizeof(wiced_bt_device_link_keys_t)) != 0 ){
//				result = WICED_BT_SUCCESS;
//			}
//    	}
    	...
    	for(uint8_t i=WICED_NVRAM_VSID_START; i<=SPP_NVRAM_ID_LIM; i++){
			if( app_read_nvram( i, &nv_read, sizeof(wiced_bt_device_link_keys_t) ) != 0 ){
				if( memcmp( &nv_read.bd_addr, &p_event_data->pairing_io_capabilities_br_edr_request.bd_addr, sizeof(wiced_bt_device_address_t) ) == 0 ){
					memcpy( &p_event_data->paired_device_link_keys_request, &nv_read, sizeof(wiced_bt_device_link_keys_t) );
					result = WICED_BT_SUCCESS;
					break;
				}
			}
...
//    }
	...
	printf("\n\n");
	printf("time: %d sec, rate: %d KB/s, avg: %d KB/s", sec, tmp/1024, avg/(sec*1024));
	printf("\n\n");
}
#endif
...
//    WICED_BT_TRACE("%s handle:%d len:%d %02x-%02x, total rx %d\n", __FUNCTION__, handle, data_len, p_data[0], p_data[data_len - 1], spp_rx_bytes);
	...
	while( data_len ){
		...
		if( data_len >> 8 ){
			wiced_hal_puart_synchronous_write( p_data, 0xFF );
			p_data += 0xFF;
			data_len -= 0xFF;
		}else{
...
#define	sz	(16)
	...
	for( i=0 ; i<sz ; i++ ){
		if( wiced_hal_puart_read(&rbuf[i]) == WICED_FALSE ){
			break;
		}
	}

	if( (spp_handle!=0) && (wiced_bt_spp_can_send_more_data(spp_handle)==WICED_TRUE) ){
    	wiced_bt_spp_send_session_data(spp_handle, (uint8_t *)&rbuf, i);
    }
